...
locals {
  rs2 = {
    ...
    tgwa = { for k,v in aws_subnet.this: v.availability_zone => v if ((var.routing_scenario==2) && strcontains(k, "tgwa-")) }
  }
}
...
resource "aws_route_table" "rs2-tgwa" {
  count = var.routing_scenario==2 ? 1 : 0

  vpc_id = aws_vpc.this.id
  tags = {
    Name = "${var.name}-rs2-tgwa"
  }
}

resource "aws_route" "rs2-tgwa--workload" {
  for_each = local.rs2.workload

  route_table_id         = aws_route_table.rs2-tgwa[0].id
  destination_cidr_block = each.value.cidr_block
  vpc_endpoint_id        = aws_vpc_endpoint.rs2-gwlbe[each.key].id
}

resource "aws_route_table_association" "rs2-tgwa" {
  for_each = local.rs2.tgwa
...
resource "aws_route" "rs2-workload-mgmt" {
  for_each = local.rs2.workload
