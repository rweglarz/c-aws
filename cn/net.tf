module "vpc_eks" {
  ...
  subnets = {
    ...
    "k8s-a" : { "idx" : 3, "zone" : var.availability_zones[0] },
    "k8s-b" : { "idx" : 4, "zone" : var.availability_zones[1] },
  }
}
...
resource "aws_route_table_association" "k8s" {
  for_each = { for k, v in module.vpc_eks.subnets : v.availability_zone => v if length(regexall("k8s-", k)) > 0 }
